...
|語彙|出典|確定していること / 不明点|
|---|---|---|
|語彙データ|docs/request.md, docs/requirement.md, docs/specs.md|辞書キーごとに Entry の配列を持つJSONデータ。|
|Entry|docs/specs.md|term/meanings/examples を持つ。dictionary は保持せず辞書キーから導出される。|
|Dictionary|docs/specs.md|source/target の組で表現される。不明: 辞書切り替え時の永続化方法。|
...
|source / target|docs/specs.md|Dictionary の属性名。|
|dictionaryKey|docs/specs.md|source/target を `source:target` で結合した文字列。|
...
|語彙|出典|確定していること / 不明点|
|---|---|---|
|Entry の一意性|docs/specs.md|dictionaryKey + term で一意。|
